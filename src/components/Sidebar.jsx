...
const Sidebar = () => {
  ...
  const renderMenuItem = (item) => {
    if (item.header) {
      return (
        <li className="menu-header small text-uppercase" key={item.key}>
          <span className="menu-header-text" data-i18n={item.i18n || item.title}>{item.title || item.header}</span>
        </li>
      );
    }

    const hasChildren = item.children && item.children.length > 0;
    const isActiveItem = hasChildren 
      ? item.children.some(child => isActive(child.path) || (child.children && child.children.some(subChild => isActive(subChild.path))))
      : isActive(item.path);
    
    const menuItemClasses = `menu-item ${isActiveItem ? 'active' : ''} ${hasChildren && isActiveItem ? 'open' : ''} ${item.active ? 'active' : ''}`;
    
    if (hasChildren) {
      return (
        <li className={menuItemClasses} key={item.key}>
          <a 
            href="#" 
          ...
          >
            ...
            <div data-i18n={item.i18n || item.title}>{item.title}</div>
            {item.badge && (
              <div className={item.badge.class}>{item.badge.text}</div>
            )}
          </a>
      ...
      return (
        <li className={menuItemClasses} key={item.key}>
          {item.path ? (
            <Link to={item.path} className="menu-link" {...linkProps}>
              {item.icon && <i className={`menu-icon tf-icons ${item.icon}`}></i>}
              <div data-i18n={item.i18n || item.title}>{item.title}</div>
            </Link>
          ) : (
            <a href="#" className="menu-link" onClick={(e) => e.preventDefault()} {...linkProps}>
              {item.icon && <i className={`menu-icon tf-icons ${item.icon}`}></i>}
              <div data-i18n={item.i18n || item.title}>{item.title}</div>
            </a>
          )}
  ...
  <g id="Pages" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
    <g id="Login---V2" transform="translate(-667.000000, -290.000000)">
      ...
      <div className="menu-scrollable mt-2" style={{ maxHeight: 'calc(100vh - 150px)', overflowY: 'auto' }}>
        <ul className="menu-inner py-1">
          {menuConfig.map(renderMenuItem)}
      </ul>
    </div> {/* Closing div for menu-scrollable */}
    </aside>
  );
};
